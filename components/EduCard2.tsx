...
import bsc from '../assets/EduCard2.jpg'
import { motion } from 'framer-motion'

type Props = {}

const EduCard2 = (props: Props) => {
  return (
    <article className='flex flex-col rounded-lg items-center space-y-7 flex-shrink-0  w-[400px] md:w-[500px] xl:w-[600px] snap-center bg-[#292929] p-10 hover:opacity-100 opacity-40 cursor-poiter transition-opacity duration-200 overflow-hidden'>
        ...
        <h4 className='text-4xl font-light '>UNDERGRADUATE</h4>
        <p className= 'text-[#c13f3f] font-bold text-2xl mt-1'>B.Sc Computer Science</p>
        <p className='uppercase py-5 text-gray-300'>From 2019 - 2022</p>
        <ul className=' space-y-4 ml-5 text-lg'>
            <li>CGPA{" "}:<span className='text-[#c13f3f] '>{" "}8.00{" "}</span></li>
            <li>A.V.C College{"("}Autonomous{")"},</li>
            <li>Mannampandal.</li>
        </ul>
        </div>
    </article>
